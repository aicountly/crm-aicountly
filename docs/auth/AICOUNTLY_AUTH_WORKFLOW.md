# AICOUNTLY auth workflow (CRM)

How CRM signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `crm.aicountly.com` (or `crm.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/crm?returnUrl={origin}/auth/callback`.
...
| CRM host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `crm.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `crm.aicountly.com/api` |
| `crm.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `crm.gh.aicountly.com/api` |
...
`crm` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://crm.gh.aicountly.com/api/health
...
curl -i -X POST https://crm.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://crm.gh.aicountly.com/api/global/login   # expect 404
```
